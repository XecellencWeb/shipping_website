...
import { DataRepresentor } from './HeadData'
...
const BodyData = ({data,userView}:DataRepresentor) => {
  ...
  return (
    ...
    <div className="flex relative">
              {Object.entries(dataRearranged)?.map(
                ([key,value],index)=>(
                  <div className="w-[16.66%] border-2 border-b-none border-solid border-[rgba(204,204,204,.3)] text-center cursor-pointer">
                  <p key={index} className="text-base mt-4 h-40 pb-8 w-full opacity-50  px-2 break-words inline-block gap-2 justify-center items-center">{formatObjectKey(value?.toString())}
                      {
                        key === 'status' && (
            ...
            <button onClick={detailsView} className=" italic text-blue-700 text-base hover:underline">{!fullView?"View Details":"Less Details"}</button>
            {!userView && (<>
